...
namespace DuneBot.Tests
{
    public class GameEngineTests
    {
        ...
        public async Task AdvancePhase_FromChoam_ShouldStartBidding_AndCreateThread()
        {
            ...
            Assert.Equal("Lasgun", game.State.CurrentCard);
        ...
        public async Task PlaceBid_ShouldUpdateState_AndPostToThread()
        {
            ...
            var game = new Game
            {
                GuildId = 100, State = new GameState
                {
        ...
        public async Task AdvancePhase_ChoamCharity_ShouldTopUpSpice()
        {
            ...
            _mockRepo.Setup(r => r.GetGameAsync(gameId)).ReturnsAsync(game);
            _mockDecks.Setup(d => d.Draw(It.IsAny<List<string>>(), It.IsAny<List<string>>())).Returns("Lasgun");
            _mockDiscord.Setup(d => d.CreatePhaseThreadAsync(It.IsAny<ulong>(), It.IsAny<ulong>(), It.IsAny<string>()))
                .ReturnsAsync(111ul);
        ...
        public async Task Battle_TraitorWin_ShouldEndBattleImmediately()
        {
            ...
            var f1 = new FactionState
                { PlayerDiscordId = 1, PlayerName = "Atreides", Faction = Faction.Atreides, Spice = 0 };
            f1.Traitors.Add("Baron Harkonnen");

            var f2 = new FactionState
                { PlayerDiscordId = 2, PlayerName = "Harkonnen", Faction = Faction.Harkonnen, Spice = 10 };
            ...
            game.State.PendingBattles
                .Enqueue(battle); // Just to satisfy check if I used it logic.. wait I didn't use Pending check for logic
        ...
        public async Task WinCondition_ThreeStrongholds_ShouldEndGame()
        {
            ...
            var s1 = new Territory { Name = "Arrakeen", IsStronghold = true };
            s1.FactionForces[Faction.Atreides] = 1;
            var s2 = new Territory { Name = "Carthag", IsStronghold = true };
            s2.FactionForces[Faction.Atreides] = 1;
            var s3 = new Territory { Name = "Sietch Tabr", IsStronghold = true };
            s3.FactionForces[Faction.Atreides] = 1;
        ...
        public async Task StartBidding_ShouldNotifyAtreides()
        {
            ...
            // Assert
            _mockDiscord.Verify(d => d.SendDirectMessageAsync(10, It.Is<string>(s => s.Contains("Lasgun"))),
                Times.Once);
        }
        ...
        public async Task StartGame_ShouldGiveHarkonnenFourTraitors()
        {
            ...
            for (int i = 2; i <= 6; i++)
                game.State.Factions.Add(new FactionState { PlayerDiscordId = (ulong)i, PlayerName = $"P{i}" });
        ...
        public async Task ResolveAuction_ShouldPayEmperor_WhenOtherWins()
        {
            ...
            var f1 = new FactionState
                { PlayerDiscordId = 1, PlayerName = "Atreides", Faction = Faction.Atreides, Spice = 10 };
            var f2 = new FactionState
                { PlayerDiscordId = 2, PlayerName = "Emperor", Faction = Faction.Emperor, Spice = 0 };
        ...
        public async Task ShipForces_ShouldApplyGuildRules()
        {
            ...
            var f1 = new FactionState
                { PlayerDiscordId = 1, PlayerName = "Atreides", Faction = Faction.Atreides, Spice = 10, Reserves = 10 };
            var f2 = new FactionState
                { PlayerDiscordId = 2, PlayerName = "Guild", Faction = Faction.Guild, Spice = 10, Reserves = 10 };
        ...
        public async Task Voice_ShouldPreventAction()
        {
            ...
            var f2 = new FactionState
            {
                PlayerDiscordId = 2, PlayerName = "Harko", Faction = Faction.Harkonnen,
                DeadLeaders = new List<string>(), TreacheryCards = new List<string> { "Lasgun" }
            };

            game.State.Factions.Add(f1);
            game.State.Factions.Add(f2);
            game.State.Map.Territories.Add(new Territory
                { Name = "Arrakeen", FactionForces = new Dictionary<Faction, int> { { Faction.Harkonnen, 5 } } });
        ...
        public async Task Prescience_ShouldRevealOpponentPlan()
        {
            ...
            var f2 = new FactionState
            {
                PlayerDiscordId = 2, PlayerName = "Harkonnen", Faction = Faction.Harkonnen,
                TreacheryCards = new List<string> { "Lasgun" }
            };
        ...
        public async Task Harkonnen_ShouldCaptureLeader_IfWinner()
        {
            ...
            var f2 = new FactionState
            {
                PlayerDiscordId = 2, PlayerName = "Atreides", Faction = Faction.Atreides
            }; // Leader "Duncan" assumed valid
            ...
            {
                Name = "Arrakeen",
                FactionForces = new Dictionary<Faction, int> { { Faction.Harkonnen, 5 }, { Faction.Atreides, 5 } }
            });
