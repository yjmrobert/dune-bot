import { GameState, LeaderState } from "../types";

export class RevivalEngine {
     ...
     */
    reviveForces(state: GameState, factionId: string, count: number): string {
        const faction = state.factions.find(f => f.playerDiscordId === factionId);
        if (!faction) throw new Error("Player not found.");

        if (count <= 0) throw new Error("Must revive at least 1 force.");
        if (count > 3) throw new Error("Cannot revive more than 3 forces per turn.");
        if (faction.forcesInTanks < count) throw new Error(`Not enough forces in tanks. You have ${faction.forcesInTanks}.`);
        ...
        if (faction.spice < cost) throw new Error(`Not enough spice. Cost is ${cost}.`);
    ...
    reviveLeader(state: GameState, factionId: string, leaderName: string): string {
        const faction = state.factions.find(f => f.playerDiscordId === factionId);
        if (!faction) throw new Error("Player not found.");

        const leader = faction.leaders.find(l => l.name === leaderName);
        if (!leader) throw new Error("Leader not found.");
        if (!leader.isDead) throw new Error("Leader is not in the tanks.");

        // Rule: Can only revive leader if ALL leaders are in tanks (dead).
        const aliveLeaders = faction.leaders.filter(l => !l.isDead);
        if (aliveLeaders.length > 0) throw new Error("Cannot revive leader while others are alive.");
    ...
    private getFreeRevivalLimit(faction: string): number {
        if (faction === "Fremen") return 3;
        if (faction === "Atreides" || faction === "Harkonnen") return 2;
        return 1;
    }
}
