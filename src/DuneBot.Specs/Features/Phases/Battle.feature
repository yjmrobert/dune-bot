Feature: Battle Resolution
  ...
  Scenario: Lasgun vs Shield - Atomic Explosion (Attacker Lasgun)
    Given the game is in the "Battle" phase
    Given the following factions are in a battle in "Arrakeen":
      | Faction   | Leader | Strength | Weapon | Defense | Dial |
      | Atreides  | Duncan |        5 | Lasgun | None    |    5 |
      | Harkonnen | Beast  |        5 | None   | Shield  |    5 |
    When the battle is resolved
    Then the battle should result in a tie
    And "Duncan" should be in dead leaders for "Atreides"
    And "Beast" should be in dead leaders for "Harkonnen"
    And "Atreides" should lose all forces in "Arrakeen"
    And "Harkonnen" should lose all forces in "Arrakeen"

  Scenario: Lasgun vs Shield - Atomic Explosion (Defender Lasgun)
    Given the game is in the "Battle" phase
    Given the following factions are in a battle in "Carthag":
      | Faction   | Leader | Strength | Weapon | Defense | Dial |
      | Atreides  | Duncan |        5 | None   | Shield  |    5 |
      | Harkonnen | Beast  |        5 | Lasgun | None    |    5 |
    When the battle is resolved
    Then the battle should result in a tie
    And "Duncan" should be in dead leaders for "Atreides"
    And "Beast" should be in dead leaders for "Harkonnen"
    And "Atreides" should lose all forces in "Carthag"
    And "Harkonnen" should lose all forces in "Carthag"
