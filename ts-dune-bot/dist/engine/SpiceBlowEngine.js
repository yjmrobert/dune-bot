...
exports.SpiceBlowEngine = void 0;
const BoardService_1 = require("../services/BoardService");
class SpiceBlowEngine {
    resolveSpiceBlow(state, nextCardOverride) {
        if (!state.spiceDeck)
            state.spiceDeck = []; // Safety
        if (!state.spiceDiscard)
            state.spiceDiscard = [];
    ...
    handleTerritory(state, card) {
        ...
        // Simple check:
        // Simple check:
        if (BoardService_1.BoardService.isInStorm(state, territoryName)) {
    ...
    clearTerritory(state, territoryName) {
        ...
        if (tState) {
            ...
            Object.keys(forces).forEach(f => {
                ...
                if (count > 0) {
                    ...
                    BoardService_1.BoardService.removeForce(state, territoryName, f, count);
                }
            });
        }
    }
}
exports.SpiceBlowEngine = SpiceBlowEngine;
